...
prepWorkspace <- function(nreps,nyears){
  ...
  # y=5;dir="setup_files/initabund_2023_05_13";useold=F    # KTS: now reverted to raster # KTS: now burns in separately for each climate scenario
  params <- doBurnin(params,y=50,dir="setup_files/initabund_2023_05_13",useold=T)
...
makescenlist <- function(params){
  scenlist <- list()
  scenlist$phi.a_pr <- c(T,F)     # does adult survival depend directly on precipitation? (add juv?)
...
doBurnin <- function(params,y,dir="setup_files/initabund_2023_05_13", useold=T){
  ...
  if(useold){
    ...
    # thispar = thisparams; nstages=params$NSTAGES; stagenames = names(params$INITABUND);thisdat = params$paraminfo
    ...
    for(clim in 1:length(params$clim_scenarios)){
      ...
      for(yr in 2:y){   
        ...
        baseclim = data.frame(mar_mar_pr_ = 0,
                              mar_apr_mtr_ = 0,
                              spr_tmean_ = 0,
                              nov_apr_pr_ = 0,
                              ann_tmin_ = 0,
...
GenerateMatrixSampler <- function(thispar,nstages,stagenames,thisdat,thisscen,vitals=F){   
  ...
  matrix_sampler <- function(climvars,yeareff=0){
    ...
    thisbasesurv <- plogis(qlogis(thispar$PHI_A$PHI_A_intercept$this) + varpart*yeareff + varpart*climvars[["spateff"]]) 
    thisPHI_A = max(0.75,min(0.999, plogis(qlogis(thisbasesurv) + thispar$PHI_A$phia.precipeff.mn$this * climvars[["nov_apr_pr_norm_"]] * thisscen$phi.a_pr) ))  # OKAY- NO NEED FOR STDEV  KTS: update with Steve's model- updatate param table with sd, make sure to implement standardization here
