...
# add Steve Hromada survival model
...
if(!cluster){
  run_name<-"5_15_2023_test" #set the run name
  arrayID<-1 #only one array if not on cluster
  if(!dir.exists(run_name)) dir.create(run_name) #make an output directory
...
if(cluster){
  ...
  #use fastscratch for faster writing
  fastscratch=T
  
  #make a folder for each set of scenarios (ie one folder per .sh submission file)
  if(fastscratch) {simspath<-sprintf("/fastscratch/kjloope/%s/sims_%s",run_name,arrayID)} else {
    simspath<-sprintf("%s/sims_%s",run_name,arrayID)}
  dir.create(simspath)
  ...
  #folder for output scenario data
  if(fastscratch) {scenpath<-sprintf("/fastscratch/kjloope/%s/scendata_%s",run_name,arrayID)} else {
    scenpath<-sprintf("%s/scendata_%s",run_name,arrayID)}
  dir.create(scenpath)
  
  #folder for output poparray data
  if(fastscratch) {poppath<-sprintf("/fastscratch/kjloope/%s/poparray",run_name)} else {
    poppath<-sprintf("%s/poparray",run_name)}
  dir.create(poppath)
}
...
if(arrayID==1){
  params <- prepWorkspace(nreps=4,nyears = 89)
...
if(cluster){
  ...
  allruns <- allruns[1:16,]    # for debugging
  write.csv(allruns, file=sprintf("%s/allruns.csv",run_name),row.names = F)
...
temp <- foreach(ndx=ndx_start:ndx_end,.packages = c('terra','raster','tryCatchLog'))%dopar% {
  ...
  thisclim <- params$allscenarios$clim_scenario[thisrep]
...
if(length(list.files(poppath, pattern = 'rds'))==nrow(allruns)){source("DT_Make_Repdf_source.R")}
